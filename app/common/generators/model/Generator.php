...
class Generator extends \yii\gii\Generator
{
        ...
        foreach ($db->getSchema()->getTableSchemas($schemaName) as $table) {
            ...
            if (stripos($table->name,"{$this->tableName}_has_")!==false
              AND $this->checkPivotTable($table) !== false){
                ...
                $pref = $this->generateNameRelations((array)$db->getTableSchema($table->name),$db->getTableSchema($this->tableName));
                
                $pref_plural = Inflector::pluralize($pref);

                $labels["{$pref}_list"] = "Список из таблицы ({$table->name})";
    ...
    public function generateNameRelations($tSchema, $CurTableSchema)
    {

        foreach($tSchema['foreignKeys'] as $k=>$v){
            if ($v[0]!= $this->tableName) {
        ...
        return lcfirst($this->generateRelationName(array(),$className, $CurTableSchema, $linkToPk[0], true));
